<template>
  <UDashboardPanel :id="`article-${id}`">
    ...
    <template #body>
      <div class="max-w-3xl p-4 sm:p-6">
        ...
        <div v-else-if="article" class="space-y-4">
          <div class="flex flex-wrap items-center gap-2">
            <span class="text-muted text-sm">Publication Vinted</span>
          ...
          <GoupixDexArticleForm mode="edit" :article="article" :loading="submitting" @submit-edit="onSubmitEdit" />
        </div>
      </div>
    </template>
  </UDashboardPanel>
...
async function publishEbay(): Promise<void> {
...
async function onSubmitEdit(body: ArticleUpdateBody): Promise<void> {
