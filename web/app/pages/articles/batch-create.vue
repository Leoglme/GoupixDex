          ...
          <div class="border-primary/20 bg-primary/5 space-y-4 rounded-xl border p-4 sm:p-5">
            ...
            <div class="grid gap-3 sm:grid-cols-[220px_minmax(0,1fr)] sm:items-end">
              <UFormField label="Photos par article">
                <UInput v-model="photosPerArticle" type="number" min="1" step="1" class="w-full" />
              </UFormField>
...
const formSlots: Ref<Array<{ id: number }>> = ref([{ id: 0 }])
const formRefs: Ref<(ArticleFormExpose | null)[]> = ref([])
const scanInputKeys: Ref<number[]> = ref([0])
const slotHints: Ref<string[]> = ref([''])
const slotScanFiles: Ref<(File | null)[]> = ref([null])
const slotScanPreviews: Ref<(string | null)[]> = ref([null])
const batchVinted: Ref<boolean> = ref(true)
const batchEbay: Ref<boolean> = ref(false)
const wardrobeImportLoading: Ref<boolean> = ref(false)
const submitting: Ref<boolean> = ref(false)
const scanning: Ref<boolean> = ref(false)
const bulkInputKey: Ref<number> = ref(0)
const photosPerArticle: Ref<number> = ref(2)
const bulkScanInput: Ref<HTMLInputElement | null> = ref(null)

function clickBulkScanInput(): void {
  bulkScanInput.value?.click()
}

function setSlotHint(idx: number, value: string): void {
  const next = [...slotHints.value]
  next[idx] = value
  slotHints.value = next
}

function bindFormRef(idx: number, el: unknown): void {
...
function removeForm(idx: number): void {
...
async function runScanIntoForm(targetIndex: number, addImage: boolean): Promise<void> {
...
async function onSlotScanFile(idx: number, file: File): Promise<void> {
...
async function onSlotHintBlur(idx: number): Promise<void> {
...
async function onBulkScanFiles(e: Event): Promise<void> {
