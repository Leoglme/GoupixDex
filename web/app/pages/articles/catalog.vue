  ...
  <UDashboardPanel id="articles-catalog-page">
    <template #header>
      <UDashboardNavbar title="Catalogue Pokémon">
        ...
        <template #right>
          <UButton to="/articles/create" color="neutral" variant="ghost" icon="i-lucide-scan-line"> Scan </UButton>
          <UButton to="/articles/stock" color="neutral" variant="ghost" icon="i-lucide-package"> Stock </UButton>
        </template>
      </UDashboardNavbar>
    </template>
        ...
        >
          ...
          <div class="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div class="max-w-2xl space-y-3">
              <p class="text-primary text-sm font-medium">Données TCGdex · prix PokéWallet</p>
              ...
              <p class="text-muted text-base leading-relaxed">
                Parcourez d’abord les <strong class="text-highlighted font-medium">séries</strong> (Bloc, Écarlate et
                Violet…), ouvrez une extension puis une carte : le formulaire se remplit (titre, description, code set,
                numéro, image HD). La recherche filtre séries et extensions. Les tarifs Cardmarket / TCGPlayer ne sont
                demandés à PokéWallet qu’au moment du clic.
              </p>
            </div>
            <div class="flex shrink-0 gap-3 lg:flex-col lg:items-end">
              ...
              >
                <UIcon name="i-lucide-library" class="text-primary size-8" />
                <UIcon name="i-lucide-sparkles" class="text-highlighted size-8" />
              </div>
            </div>
          </div>
        </div>
        ...
        <UCard class="ring-default/60 shadow-sm ring-1" :ui="{ body: 'sm:p-8 p-6' }">
          <template #header>
            <div class="flex flex-wrap items-center justify-between gap-3 px-1">
              <div class="min-w-0 space-y-1">
                <p class="text-muted text-sm">Parcours guidé</p>
                <p class="text-highlighted text-lg font-semibold">Catalogue et fiche article</p>
                <p v-if="selectedSet" class="text-muted text-sm">
                  Extension sélectionnée : <span class="text-highlighted font-medium">{{ selectedSet.name }}</span>
                </p>
                <p v-else-if="activeSeriesId" class="text-muted text-sm">
                  Série : <span class="text-highlighted font-medium">{{ activeSeriesName }}</span>
                </p>
              </div>
              <div class="flex shrink-0 flex-wrap items-center gap-2">
                ...
                <UBadge v-if="selectedSet?.abbreviation?.official" color="primary" variant="subtle" size="md">
          ...
          <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
            <div class="flex flex-wrap items-end gap-3">
              ...
              <UFormField label="Recherche" class="max-w-md min-w-[12rem] flex-1">
                <UInput
                  v-model="search"
          ...
          <div class="grid gap-8 lg:grid-cols-2 lg:items-start lg:gap-10">
            <div class="min-w-0 space-y-4">
              <div class="border-default flex items-center gap-3 border-b pb-3">
                <div class="bg-primary/15 flex size-12 items-center justify-center rounded-xl">
                  <UIcon name="i-lucide-library" class="text-primary size-7" />
                </div>
                <div class="min-w-0">
              ...
              <div v-if="(seriesLoading || setsLoading) && !selectedSet && !activeSeriesId" class="space-y-3 py-2">
                <p class="text-muted text-sm">Chargement du catalogue…</p>
                <UProgress animation="carousel" />
              </div>

              <div v-else-if="seriesDetailLoading && !selectedSet" class="space-y-3 py-2">
                <p class="text-muted text-sm">Chargement des extensions…</p>
                <UProgress animation="carousel" />
              </div>

              <div v-else-if="setLoading" class="space-y-3 py-2">
                <p class="text-muted text-sm">Chargement des cartes…</p>
                <UProgress animation="carousel" />
              </div>

              <div v-else-if="selectedSet" class="space-y-4">
                <UButton color="neutral" variant="soft" size="sm" icon="i-lucide-arrow-left" @click="backToSets">
                  {{
                    activeSeriesId
                ...
                <div class="grid max-h-[min(62vh,560px)] grid-cols-2 gap-3 overflow-y-auto pr-1 sm:grid-cols-3">
                  <UCard
                    v-for="c in catalogCardsDisplay"
                    :key="c.id"
                    variant="subtle"
                    class="cursor-pointer overflow-hidden transition-shadow hover:shadow-md"
                    :class="{ 'pointer-events-none opacity-60': previewLoading }"
                    tabindex="0"
                    role="button"
                  ...
                  >
                    <div class="space-y-2 p-2">
                      <div class="bg-muted/30 aspect-[63/88] w-full overflow-hidden rounded-lg">
                        <img
                          v-if="c.thumbUrl"
                          :src="c.thumbUrl"
                          :alt="c.name"
              ...
              <div v-else-if="activeSeriesId" class="space-y-4">
                <UButton color="neutral" variant="soft" size="sm" icon="i-lucide-arrow-left" @click="backToSeriesList">
                  Retour aux séries
                </UButton>

                <div
                  v-if="!setsInSeries.length && !seriesDetailLoading"
                  class="border-default text-muted rounded-xl border border-dashed px-4 py-10 text-center text-sm"
                >
                  Aucune extension dans cette série.
                </div>

                <div v-else class="max-h-[min(62vh,560px)] space-y-3 overflow-y-auto pr-1">
                  <UCard
              ...
              <div v-else class="space-y-6">
                ...
                <template v-if="!search.trim() && seriesSorted.length">
                  <div class="max-h-[min(62vh,560px)] space-y-3 overflow-y-auto pr-1">
                    <UCard
                ...
                <template v-else-if="search.trim()">
                  <div v-if="seriesSorted.length" class="max-h-[min(30vh,280px)] space-y-2 overflow-y-auto pr-1">
                    <p class="text-muted text-xs font-semibold tracking-wide uppercase">Séries</p>
                    <div class="space-y-2">
                      <UCard
                        v-for="ser in seriesSorted"
                        :key="ser.id"
                      ...
                      >
                        <div class="flex min-w-0 items-center gap-3 p-3">
                          ...
                          <div class="min-w-0 flex-1">
                            <p class="text-highlighted truncate text-sm leading-snug font-medium">
                              {{ ser.name }}
                            </p>
                  ...
                  <div v-if="sets.length" class="max-h-[min(42vh,400px)] space-y-2 overflow-y-auto pr-1">
                    <p class="text-muted text-xs font-semibold tracking-wide uppercase">Extensions</p>
                    <div class="space-y-2">
                      <UCard
                        v-for="s in sets"
                        :key="s.id"
            ...
            <div class="min-w-0 space-y-4">
              <div class="border-default flex items-center gap-3 border-b pb-3">
                <div class="flex size-12 items-center justify-center rounded-xl bg-neutral-500/15">
                  <UIcon name="i-lucide-file-pen-line" class="text-highlighted size-7" />
                </div>
                <div>
                  <h2 class="text-highlighted text-lg font-semibold">Fiche article</h2>
                  <p class="text-muted text-sm">
              ...
              <UCard variant="subtle" class="ring-default/40 ring-1">
...
import type { CatalogLocale, TcgdexSeriesBrief, TcgdexSetBrief, TcgdexSetDetail } from '~/composables/useCardCatalog'
...
const seriesSorted = computed(() => {
  ...
  return [...seriesList.value].sort((a, b) => a.name.localeCompare(b.name, loc, { sensitivity: 'base' }))
})
