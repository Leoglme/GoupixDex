<template>
  <div class="max-w-3xl space-y-6">
    <div>
      <UButton to="/settings" color="neutral" variant="ghost" icon="i-lucide-arrow-left"> Retour </UButton>
    </div>
    <div class="space-y-2">
      <div>
    ...
    <div v-if="loading || !s" class="text-muted py-8 text-sm">Chargement…</div>

    <template v-else>
      <UCard>
        <template #header>
          <p class="text-highlighted font-medium">Canaux</p>
        </template>
        <div class="space-y-4">
          ...
          <p class="text-muted text-sm">
            eBay est configuré pour <strong>eBay France</strong> uniquement (cartes Pokémon, envoi depuis votre
            adresse).
          </p>
          <UButton :loading="saving" @click="saveChannels"> Enregistrer les canaux </UButton>
        </div>
      </UCard>

      <GoupixDexVintedAccountCard v-if="s.vinted_enabled" />
      ...
      <UCard v-if="s.ebay_enabled && s.ebay_oauth_configured">
        <template #header>
          <p class="text-highlighted font-medium">eBay France</p>
          <p class="text-muted text-sm">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <UButton v-if="!s.ebay_connected" icon="i-lucide-link" @click="startEbayOAuth"> Se connecter à eBay </UButton>
      ...
      <UCard v-if="s.ebay_enabled && s.ebay_connected && s.ebay_listing_config_complete" class="border-success/30">
        <p class="text-highlighted font-medium">Prêt pour eBay</p>
      ...
      <UCard v-if="s.ebay_enabled && s.ebay_connected && !s.ebay_listing_config_complete">
        <template #header>
          <p class="text-highlighted font-medium">Terminer la configuration (2 minutes)</p>
        </template>

        <div class="space-y-6">
          <div class="text-muted space-y-3 text-sm">
            <p class="text-highlighted font-medium">Étape 1 — Connexion</p>
            <p>Vous êtes connecté à eBay. Nous utilisons votre compte uniquement pour publier vos cartes.</p>

            <p class="text-highlighted pt-2 font-medium">Étape 2 — Adresse d'expédition</p>
            <p>
              Indiquez l'adresse d'où vous postez vos colis. Nous créons automatiquement l'emplacement et les règles
              eBay (livraison France, paiement, retours) sans que vous alliez sur le site eBay.
            </p>
          </div>
      ...
      <UCard v-else-if="s.ebay_enabled && !s.ebay_connected">
        <p class="text-muted text-sm">Connectez votre compte eBay ci-dessus pour lancer l'assistant.</p>
      </UCard>

      <UCard v-else-if="!s.ebay_enabled">
        <p class="text-muted text-sm">Activez eBay dans « Canaux » pour configurer la vente sur eBay France.</p>
      </UCard>
    </template>
  </div>
</template>
...
useGoupixPageSeo('Paramètres — marketplace', 'Gérez vos marketplaces de vente et choisissez où publier vos annonces.')
