    ...
    <div class="flex flex-wrap items-end justify-between gap-3">
      <div>
        ...
        >
          ...
          <span>
            {{ outliersExcluded }} annonce<span v-if="outliersExcluded > 1">s</span> hors-marché exclue<span
              v-if="outliersExcluded > 1"
              >s</span
            >
            du calcul
          </span>
      ...
      <UCard v-for="card in cards" :key="card.label" :ui="{ body: 'p-4 sm:p-5' }" class="ring-default/60 ring-1">
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0 space-y-0.5">
            ...
            <p class="text-highlighted text-lg font-semibold tabular-nums sm:text-xl">
              {{ card.value }}
            </p>
          </div>
        </div>
...
/**
 * Format EUR stats row values (dash when unknown).
 * @param value - Amount or null/undefined
 * @returns {string} Currency string or em dash
 */
...
const cards: ComputedRef<StatCard[]> = computed(() => [
  {
    label: 'Prix minimum',
