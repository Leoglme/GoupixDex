<template>
  <div class="relative mx-auto w-full max-w-[13.5rem] select-none" :data-phase="vphase" aria-hidden="true">
    <div class="relative mx-auto aspect-[5/7] w-36 overflow-visible py-1 sm:w-40">
      <div
        v-for="s in slotIndices"
        :key="s"
        class="bg-elevated absolute inset-0 flex flex-col overflow-hidden rounded-xl transition-all ease-in-out"
        :class="[slotTiming(s), poseForSlot(s)]"
        :style="{ zIndex: zForSlot(s) }"
      >
        <div class="relative m-2 flex flex-1 flex-col rounded-lg bg-linear-to-br" :class="accentForSlot(s)">
          <div class="bg-default/40 m-2 h-2 w-8 rounded-full" />
          <div class="mx-2 mt-1 flex flex-1 flex-col justify-center gap-1.5 px-2">
            <div class="bg-default/30 h-1.5 w-full rounded" />
            <div class="bg-default/25 h-1.5 w-[80%] rounded" />
            <div class="bg-default/20 h-1.5 w-[60%] rounded" />
          </div>
        </div>
        <div class="border-default/60 bg-default/40 flex items-center justify-between gap-2 border-t px-2.5 py-2">
          <div class="bg-default/35 h-2 w-12 rounded" />
        ...
        >
          <div
            ...
            class="bg-default/50 absolute inset-0 z-20 flex flex-col items-center justify-center gap-2 rounded-xl backdrop-blur-[3px]"
          >
            <span
              class="bg-success/20 text-success ring-success/50 flex size-11 items-center justify-center rounded-full ring-2"
            >
              <UIcon name="i-lucide-check" class="size-6" />
            </span>
            <UBadge color="success" variant="subtle" size="sm" class="font-semibold"> Vendu </UBadge>
          </div>
        </Transition>
      </div>
    </div>
...
const phase: Ref<Phase> = ref('stack')
const reduceMotion: Ref<boolean> = ref(false)

/** Role of each physical slot 0,1,2 (real stack — rotates after each sale). */
const slotRole: Ref<StackRole[]> = ref(['back', 'mid', 'front'])
const slotVariant: Ref<number[]> = ref([0, 1, 2])
let nextVariantId = 3

const teleportSlot: Ref<number | null> = ref(null)
const noTransition: Ref<boolean> = ref(false)
const saleCycle: Ref<number> = ref(0)
...
const ringByVariant = ['ring-default/70', 'ring-primary/40', 'ring-secondary-400/45'] as const

const footerAccents = ['bg-primary/45', 'bg-secondary-400/50', 'bg-primary/35'] as const
...
/**
 * Z-index for stacked card slots during the animation phases.
 * @param slotIdx - Slot index (0..2)
...
/**
 * Tailwind duration classes for the slot transition (exit phase uses longer easing).
 * @param slotIdx - Slot index
 * @returns {string} Duration utility classes
 */
function slotTiming(slotIdx: number): string {
...
/**
 * Transform / ring classes for a slot given motion preferences and phase.
 * @param slotIdx - Slot index
 * @returns {string} Tailwind class string for the card wrapper
 */
...
/**
 * Whether the “sold” overlay should cover this slot.
 * @param slotIdx - Slot index
 * @returns {boolean} Show overlay
 */
function showOverlayOnSlot(slotIdx: number): boolean {
  if (reduceMotion.value) {
...
/**
 * Gradient accent class for the card header stripe.
 * @param slotIdx - Slot index
 * @returns {string} Tailwind gradient classes
 */
function accentForSlot(slotIdx: number): string {
  return accentGradients[slotVariant.value[slotIdx] % 3]
}

/**
 * Footer bar color class for the slot variant.
 * @param slotIdx - Slot index
 * @returns {string} Tailwind background class
 */
function footerForSlot(slotIdx: number): string {
