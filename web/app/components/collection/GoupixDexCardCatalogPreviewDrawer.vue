      ...
      >
        <div class="border-default flex items-start gap-2 border-b px-4 py-3 sm:px-5 sm:py-4">
          ...
          <div class="min-w-0 flex-1">
            <p class="text-muted truncate text-xs">{{ card.setName }}</p>
            <h2 class="text-highlighted truncate text-base leading-tight font-semibold">{{ card.name }}</h2>
          </div>
        ...
        <div class="min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-4 py-4 sm:px-5">
          <GoupixDexCardCatalogPreviewBody :key="card.id" :card="card" @added="(c) => emit('added', c)" />
        </div>
      </div>
    </Transition>
  </Teleport>
...
defineProps({
  ...
  card: {
    type: Object as PropType<GoupixCatalogCardRef | null>,
    default: null,
  },
  showBack: {
    type: Boolean,
