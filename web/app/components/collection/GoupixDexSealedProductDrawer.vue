<template>
  <Teleport to="body">
    <Transition name="goupix-sealed-drawer-backdrop">
      <div v-if="open" class="fixed inset-0 z-40 bg-(--app-overlay)" aria-hidden="true" @click="emit('close')" />
    </Transition>

    <Transition name="goupix-sealed-drawer-panel">
      <div
        v-if="open && sealedId"
      ...
      >
        ...
        <div class="min-h-0 flex-1 overflow-x-hidden overflow-y-auto px-4 py-4 sm:px-5">
          <GoupixDexSealedProductDetailBody
            :key="sealedId"
            :sealed-id="sealedId"
            @updated="(p) => emit('updated', p)"
...
<style scoped>
.goupix-sealed-drawer-panel-enter-active,
.goupix-sealed-drawer-panel-leave-active {
  transition: transform 0.25s cubic-bezier(0.4, 0, 0.2, 1);
}
.goupix-sealed-drawer-panel-enter-from,
.goupix-sealed-drawer-panel-leave-to {
  transform: translateX(100%);
}

.goupix-sealed-drawer-backdrop-enter-active,
.goupix-sealed-drawer-backdrop-leave-active {
  transition: opacity 0.2s ease;
}
.goupix-sealed-drawer-backdrop-enter-from,
.goupix-sealed-drawer-backdrop-leave-to {
  opacity: 0;
}
