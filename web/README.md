...
| `/articles`, `/articles/sold`                        | Vente ("Mes articles") | Two tabs: En vente / Vendus (`ARTICLES_PAGE_TABS`). `/articles/stock` redirects to `/articles`.                                                                                                                                                                                                                    |
...
| `/collection`, `/collection/add`, `/collection/scan` | Collection             | Personal binder, TCGdex catalog, live phone scan with in/out (cash-register) mode; desktop shows a QR code + live feed. Cards are identified **on-device** first (perceptual-hash match against `public/scan-index/`, built by `api/scripts/build_scan_match_index.py`), with the server OCR pipeline as fallback. |
