...
class PokeWalletReferencePricesService:
    """Pick reference prices from Cardmarket / TCGPlayer rows."""
    ...
    def pick_cardmarket_reference_eur(
        ...
        for row in ordered:
            candidates: list[float | None] = [
                PokeWalletReferencePricesService._float_or_none(row.get("avg1")),
                PokeWalletReferencePricesService._float_or_none(row.get("avg7")),
                PokeWalletReferencePricesService._float_or_none(row.get("avg30")),
                PokeWalletReferencePricesService._float_or_none(row.get("trend")),
                PokeWalletReferencePricesService._float_or_none(row.get("avg")),
                PokeWalletReferencePricesService._float_or_none(row.get("low")),
            ]
            for c in candidates:
